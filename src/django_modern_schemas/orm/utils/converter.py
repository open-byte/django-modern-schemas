...
from typing import TYPE_CHECKING, Annotated, Any, Optional, TypeVar, no_type_check
...
def assert_valid_name(name: str) -> None:
    ...
    assert COMPILED_NAME_PATTERN.match(name), f'Names must match /{NAME_PATTERN}/ but "{name}" does not.'
...
class FieldConversionProps:
    ...
    def __init__(self, field: Field):
        ...
        data['description'] = force_str(getattr(field, 'help_text', field.verbose_name)).strip()
        data['title'] = field.verbose_name.title()  # ty:ignore[unresolved-attribute]
...
@singledispatch
def convert_django_field(field: Field, **kwargs: Any) -> tuple[type, PydanticField]:
    raise Exception(f"Don't know how to convert the Django field {field} ({field.__class__})")


@no_type_check
def create_m2m_link_type(type_: type[TModel], related_model: type[models.Model]) -> type[TModel]:
    class M2MLink(type_):
        @classmethod
...
def construct_related_field_schema(
    ...
    return (
        ...
        PydanticField(
            default=default,
            description=force_str(getattr(field, 'help_text', field.verbose_name)).strip(),
...
def construct_relational_field_info(
    ...
    *,
    ...
    inner_type, field_info = convert_django_field(field.related_model._meta.pk, registry=registry, depth=depth)
...
def convert_field_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(str, field)


@no_type_check
@convert_django_field.register(models.EmailField)
def convert_field_to_email_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(EmailStr, field, is_custom_type=True)


@no_type_check
@convert_django_field.register(models.URLField)
def convert_field_to_url_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(AnyUrl, field, is_custom_type=True)


@no_type_check
@convert_django_field.register(models.AutoField)
def convert_field_to_id(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(int, field)


@no_type_check
@convert_django_field.register(models.UUIDField)
def convert_field_to_uuid(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
...
def convert_field_to_int(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(int, field)


@no_type_check
@convert_django_field.register(models.BinaryField)
def convert_field_to_byte(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
...
@convert_django_field.register(models.GenericIPAddressField)
def convert_field_to_ipaddress(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(IPvAnyAddress, field)


@no_type_check
@convert_django_field.register(models.FloatField)
def convert_field_to_float(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(float, field)


@no_type_check
@convert_django_field.register(models.DecimalField)
def convert_field_to_decimal(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(Decimal, field)


@no_type_check
@convert_django_field.register(models.BooleanField)
def convert_field_to_boolean(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(bool, field)


@no_type_check
@convert_django_field.register(models.NullBooleanField)
def convert_field_to_null_boolean(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(bool, field)


@no_type_check
@convert_django_field.register(models.DurationField)
def convert_field_to_time_delta(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(datetime.timedelta, field)


@no_type_check
@convert_django_field.register(models.DateTimeField)
def convert_datetime_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(datetime.datetime, field)


@no_type_check
@convert_django_field.register(models.DateField)
def convert_date_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    return construct_field_info(datetime.date, field)


@no_type_check
@convert_django_field.register(models.TimeField)
def convert_time_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
...
def convert_field_to_django_model(
    field: Field,
    registry: SchemaRegister | None = None,
    depth: int = 0,
    skip_registry: bool = False,
    **kwargs: DictStrAny,
) -> tuple[type, PydanticField]:
...
def convert_postgres_array_to_list(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
...
def convert_postgres_field_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    python_type = Json
    if field.null:
        python_type = Optional[Json]  # noqa: UP045
    return construct_field_info(python_type, field)


@no_type_check
@convert_django_field.register(RangeField)
def convert_postgres_range_to_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
    inner_type, field_info = convert_django_field(field.base_field)
    if not isinstance(inner_type, list):
        inner_type = list[inner_type]
    return inner_type, field_info
...
if django.VERSION >= (3, 2):
    ...
    def convert_field_to_json_string(field: Field, **kwargs: DictStrAny) -> tuple[type, PydanticField]:
        python_type = Json
        if field.null:
            python_type = Optional[Json]  # noqa: UP045
        return construct_field_info(python_type, field)
